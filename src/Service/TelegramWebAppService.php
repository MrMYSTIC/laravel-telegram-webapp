...
use Micromagicman\TelegramWebApp\Http\TmaRequest;
...
class TelegramWebAppService
{
     ...
     */
    public function verifyInitData(?TmaRequest $request = null): bool
    {
        if (!webAppConfig('enabled')) {
            return true;
        }
        $queryParams = $request?->webAppInitData();
     ...
     */
    public function getWebAppUser(?TmaRequest $request = null): ?TelegramUser
    {
        $requestQuery = $request?->webAppInitData();
        if (!array_key_exists(self::USER_QUERY_PARAMETER_KEY, $requestQuery)) {
            return null;
        }
        $telegramUserData = json_decode(
            $requestQuery[self::USER_QUERY_PARAMETER_KEY],
            true,
            512,
            JSON_THROW_ON_ERROR
        );
    ...
    private function createHashFromQueryString(array $queryParams): string
    {
        ...
        $dataWithoutHash = array_filter(
            $queryParams,
            static fn ($key) => $key !== self::HASH_QUERY_PARAMETER_KEY,
